...
def lambda_infinite():
    print('infinite lambda -- noise free : training...')
    from inputs import D_rot_list, Lambda_list
    Lambda_max = np.max(np.array(Lambda_list))
    from inputs import n_realization_traj, n_train_sample, clf, memory, grad_label
    env = conc.profile.DimensionTwo()
    k1 = int(grad_label[0]) # grad_label = [k1, k2], where k is the delay. k1<k2
    k2 = int(grad_label[1])
    column_names = ['Lambda', 'D_rot'] + ['bias'] + [f'w{i}' for i in range(memory)]
    weights_mean, weights_sem = [], []
    for D_rot in D_rot_list:
        Dtext = f'D_rot = {D_rot:<10}'
        weights_list = []
        for i in range(n_realization_traj):
            ...
            Xtrain = bind_normalized.copy()
            clf.fit(Xtrain, ytar)
            weights_list.append(weights(clf)) 
        df_weight = pd.DataFrame(weights_list)
        wmean = df_weight.mean(axis = 0).values
        wsem  = df_weight.sem(axis = 0).values
        weights_mean.append(['lambda_inf', D_rot] + list(wmean))
        weights_sem.append(['lambda_inf', D_rot] + list(wsem))
    #
    df_weights_mean = pd.DataFrame(data=weights_mean, columns=column_names)
    df_weights_sem = pd.DataFrame( data=weights_sem,  columns=column_names)
    write_dataframe(df_weights_mean, 'weights_mean_lambda_inf')
    write_dataframe(df_weights_sem, 'weights_sem_lambda_inf')
    return
...
def sample_dataset_for_plots():
    from inputs import T_max, Lambda_list, memory, n_train_sample, grad_label
    D_rot = 0.5
    Lambda = np.min(np.array(Lambda_list))
    ...
    dt = T_max/memory
    seed = 0
    conc_list, df_initial, traj_list = bact.attributes.sample_dataset(seed, env, n_train_sample, D_rot, Lambda)
    ytar = (conc_list[:, k1] - conc_list[:, k2]) > 0 # labeling
    lambda_conc_dt = Lambda*conc_list*dt
