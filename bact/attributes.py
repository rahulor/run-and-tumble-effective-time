...
def dataset(seed, env, n_sample, D_rot):
    ...
    psi_sample = diffusion_upto_tau(seed, psi_0_list, D_rot)
    conc_list = np.empty((n_sample, memory))
    agent = RunUptoTau(env)
...
def random_initial_condition(seed, n_sample):
    from inputs import traj_at_fixed_r 
    rng = np.random.RandomState(seed)
    r0 =  traj_at_fixed_r
    random_theta = rng.uniform(-np.pi, np.pi, n_sample)
    psi = rng.uniform(-np.pi, np.pi, n_sample)
    df = pd.DataFrame()
    df['x'] = r0 * np.cos(random_theta)
    df['y'] = r0 * np.sin(random_theta)
    df['psi'] = psi
    return(df)

def diffusion_upto_tau(seed, psi_0_list, D_rot):
    rng = np.random.RandomState(seed)
